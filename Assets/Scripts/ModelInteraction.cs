    ...
    void Update()
    {
        ...
        if ((Input.touchCount > 0) && (Input.GetTouch(0).phase == TouchPhase.Moved))
        {
            ...
            if (Physics.Raycast(raycast, out hit))
            {
                ...
                {
                    //Debug.Log(Input.GetTouch(0).deltaPosition.x + " " + Input.GetTouch(0).deltaPosition.y);
                    transform.Rotate(0f, 0f, t1.deltaPosition.x * speedAdj);
            ...
            if (Physics.Raycast(raycast, out hit))
            {
                ...
                if (hit.collider.tag == "ModelInteract")
                {
                    ...
                    else if (t1.phase == TouchPhase.Moved || t2.phase == TouchPhase.Moved)
                    {
                        ...
                        Debug.Log("Scales: " + moveDist + ", " + initDist + ", " + direction);
                        Debug.Log(initScale + ", " + minScale + ", " + ", " + maxScale + ", " + scale );
                        ...
                        //    if ((initScale.x > minScale.x && initScale.y > minScale.y && initScale.z > minScale.z) &&
                        //    (initScale.x < maxScale.x && initScale.y < maxScale.y && initScale.z < maxScale.z))
                        //{
                        //    initScale *= scale;
                        //    transform.localScale = initScale * scale;
                        //}
                        //transform.localScale = Vector3(initScale[0] * scale, initScale[1] * scale, initScale[2] * scale);
                    }
